...
const imageTitle = '잘못된 사진 제보';
const requestTitle = '부적절한 의뢰 신고';

const imageText =
  '의뢰자에게 사진에 어떤 문제가 있는지 구체적으로 알려주세요.\n 제보 내용은 의뢰자에게 즉시 전달됩니다.';
const requestText = '사진 및 질문이 (선정성, 폭력성, 사기 등) 부적절하다면\n 운영진에게 알려주세요.';
    ...
    <BottomSheet visible={visible} onBackButtonPress={onClose} onBackdropPress={onClose}>
      <View style={[styles.container, SigonganColor.backgroundPrimary]}>
        <View style={styles.titleWrapper}>
          <Text style={styles.titleText}>{category ? requestTitle : imageTitle}</Text>
        </View>
        <View style={SigonganDesign.borderOpaque} />
        <View style={styles.itemWrapper}>
          <View style={styles.titleContainer}>
            <Text style={[commentFont.SMALL_TITLE, styles.description]}>{category ? requestText : imageText}</Text>
          </View>

          <View style={{ width: '90%' }}>
            {category
              ? selectRequestList.map((question) => (
