...
export const ChallengePostItem = ({ post, onPress }: { post: challengePost; onPress: (post: challengePost | null) => void }) => {
    const theme = useTheme();
        ...
        <>
            <Pressable onPress={() => onPress(post)} style={{ flex: 1, flexDirection: 'column', backgroundColor: theme.COLOR.WHITE, paddingHorizontal: GET_PADDING('P4'), paddingVertical: GET_PADDING('P2'), gap: 10, borderRadius: theme.STYLES.RADIUS.lg }}>
                ...
                <View style={{ position: "relative" }}>
                    <Image source={{ uri: imageSrc }} style={{ width: '100%', height: 200, borderRadius: theme.STYLES.RADIUS.lg }} />
                    <View style={{ position: "absolute", top: 14, right: 0, backgroundColor: theme.COLOR.MINT_2, borderTopLeftRadius: theme.STYLES.RADIUS.sm, borderBottomLeftRadius: theme.STYLES.RADIUS.sm, paddingHorizontal: GET_PADDING('P6'), paddingVertical: GET_PADDING('P5') / 3 }}>
                        <Typography size="text_sm" weight="bold" color={theme.COLOR.WHITE}>도전 해설</Typography>
                    </View>
                </View>            
            </Pressable>
            <Margin margin={GET_MARGIN('h6') * 2} />
